...
class Test extends BaseCurrencyCloudVCRTestCase
{
    ...
    private static $SDK_CANNOT_AUTHORISE_LOGIN_ID = 'development+cannot.authorise@currencycloud.com';
    private static $SDK_CANNOT_AUTHORISE_API_KEY  = 'deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef';

    private static $SDK_CAN_AUTHORISE_LOGIN_ID = 'development+can.authorise@currencycloud.com';
    private static $SDK_CAN_AUTHORISE_API_KEY  = 'deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef';
    ...
    */
    public function cannotAuthoriseOwnPayment(){
      ...
      $client = $this->getClient(self::$SDK_CANNOT_AUTHORISE_LOGIN_ID, self::$SDK_CANNOT_AUTHORISE_API_KEY);
    ...
    */
    public function canAuthorisePayment(){
      ...
      // change to an account that can authorise the payment
      $client = $this->getClient(self::$SDK_CAN_AUTHORISE_LOGIN_ID, self::$SDK_CAN_AUTHORISE_API_KEY);
      ...
      foreach ($authorised_payments as $key => $authorised_payment) {
        ...
        // assert the UUID's are those that were passed in the call to authorise the payments, and the same as the payment UUID when created
        $this->assertContains($authorised_payments[$key]->getPaymentId(),$payment_ids['payment_ids']);
        // check the payment_status is 'authorised'
